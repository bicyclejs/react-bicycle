...
export default function connectErrors(options: Options = EMPTY_OBJECT) {
  return <OriginalProps extends {}>(
    WrappedComponent: Component<InjectedProps & OriginalProps>,
  ): Component<OriginalProps> => {
    let key = 0;
    class Connect extends React.Component<
      OriginalProps & {bicycleContext: Context},
      State
    > {
      ...
      constructor(props: any) {
        super(props);
        this._client = props.client || props.bicycleContext.client;
        ...
        if (!this._client) {
          throw new Error(
            `Could not find "client" in either the context or ` +
              ...
              `or explicitly pass "client" as a prop to "${
                Connect.displayName
              }".`,
          );
        }
      ...
      componentDidMount() {
        this._networkErrorsSubscription = this._client.subscribeToNetworkErrors(
          this._onNetworkError,
        );
        this._mutationErrorsSubscription = this._client.subscribeToMutationErrors(
          this._onMutationError,
        );
        if (
          this._client.subscribeToQueueRequest &&
          this._client.subscribeToSuccessfulResponse
        ) {
          this._queueRequestSubscription = this._client.subscribeToQueueRequest(
            this._onQueueRequest,
          );
          this._successfulResponseSubscription = this._client.subscribeToSuccessfulResponse(
      ...
      _onMutationError = (err: Error) => {
        ...
        this.setState({
          mutationErrors: this.state.mutationErrors.concat([err]),
        });
      };
      _onQueueRequest = () => {
      ...
      _dismissNetworkError = ({key}: {key: string}) => {
        const networkErrors = this.state.networkErrors.filter(
          err => err.key !== key,
        );
        this.setState({
      ...
      _dismissMutationError = ({key}: {key: string}) => {
        const mutationErrors = this.state.mutationErrors.filter(
          err => err.key !== key,
        );
        this.setState({
      ...
      render() {
        return React.createElement(
          WrappedComponent as Component<OriginalProps & InjectedProps>,
          {
            ...(this.props as any),
    ...
    function ExtractContextComponent(props: OriginalProps) {
      const ctx = useContext();
      return <Connect {...props} bicycleContext={ctx} />;
    }
    return hoistStatics(ExtractContextComponent, WrappedComponent);
  };
}
